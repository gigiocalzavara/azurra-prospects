...
O estado visual de cada job usa `jobMessages`, separado da mensagem global do formulario. O recarregamento do historico nao pode limpar mensagens operacionais ativas. Falhas devem aparecer dentro do cartao correspondente com um codigo seguro e sem dados do fornecedor.

O adaptador executa pesquisa de usuarios com limite maximo de 250 itens e `maxItems`, que limita a quantidade faturavel no modelo por resultado. O teto monetario opcional nao e enviado porque pode ser incompatível com alguns modelos de cobranca do executor. Enriquecimento adicional fica desabilitado por padrao. A normalizacao aplica escopo publico, faixa de seguidores e minimizacao de dados antes de persistir em `prospect_results`. Apenas nome, usuario, URLs publicas, biografia, metricas, categoria, localizacao e contatos explicitamente publicos podem ser gravados.
